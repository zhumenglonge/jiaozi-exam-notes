...
import './App.css'
import { topics, groupTopicsByCategory } from './questions/registry.js'

/* =========================================================
 * 应用根组件：只负责"目录页 <-> 题目页"的切换
 * ---------------------------------------------------------
 * 所有题目内容都在 src/questions/ 下，通过 registry.js 注册。
    ...
    <>
      <header className="app-header">
        ...
        <p className="subtitle">
          小学教师资格证考试 — 记录每一道真题与错题，反复巩固直到全部掌握
        </p>
      </header>

      <div className="home">
        <div className="home-stats">
          <span><b>{topics.length}</b> 个题组</span>
          <span><b>{groups.length}</b> 个科目</span>
        </div>

        {topics.length === 0 && (
          <div className="empty-tip">
            还没有题目，去 <code>src/questions/registry.js</code> 注册一个吧 👇
          </div>
        )}
...
function TopicPage({ topic, onBack }) {
  ...
  return (
    <>
      ...
      <Suspense fallback={<div className="loading-fallback">⏳ 题目加载中...</div>}>
        <Component />
      </Suspense>
    </>
  )
